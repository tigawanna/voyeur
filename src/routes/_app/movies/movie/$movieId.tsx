...
function MovieDetailsPage() {
  ...
  const { data: basicRows } = useLiveQuery(
    (q) => q.from({ movie: movieBasicCollection }).where(({ movie }) => eq(movie.id, id)),
    [id],
  );
  ...
  const { data: browseRows } = useLiveQuery(
    (q) => q.from({ movie: moviesCollection }).where(({ movie }) => eq(movie.id, id)),
    [id],
  );
  ...
  const { data: recommendationRows } = useLiveQuery(
    (q) => q.from({ movie: movieRecommendationsCollection }).where(({ movie }) => eq(movie.id, id)),
    [id],
  );
  ...
  const {
    ...
    (q) => q.from({ movie: movieDetailCollection }).where(({ movie }) => eq(movie.id, id)),
    [id],
  );
  ...
  const summaryRow =
    basicRows.at(0) ?? browseRows.at(0) ?? recommendationRows.at(0) ?? detailRows.at(0);
